...
class wepredict(object):
    ...
    def __init__(self, file_path: str, cluster, perc_valid, testing):
        """Allow predction via LD blocks."""
    ...
    def generate_DAG(self, phenotype, index_valid, alphas):
        """Get future objects."""
        outcome = []
        for ff in self.files:
            mat = delayed(self._reader_binary)(ff)
...
if __name__ == '__main__':
    ...
    monster = wepredict(folder+'/*', cluster, 0.1, True)
    ...
    monster.generate_DAG(phenoype, index_valid, alphas)
    out = monster.compute()
